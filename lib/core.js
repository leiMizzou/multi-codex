...
const TOKEN_WINDOW_PRESETS = [
  { key: "all", label: "All time", windowMs: null },
  { key: "1d", label: "Last 24 hours", windowMs: DAY_MS },
  { key: "7d", label: "Last 7 days", windowMs: 7 * DAY_MS },
  { key: "30d", label: "Last 30 days", windowMs: 30 * DAY_MS },
];
...
async function buildDashboardPayload(options = {}) {
  const projectHome = options.projectHome || getProjectHome();
  const generatedAtMs = toEpochMs(options.generatedAt) || Date.now();
  const generatedAt = new Date(generatedAtMs).toISOString();
  const accounts = await collectAccountStatuses({
    ...options,
    now: generatedAtMs,
  });
  ...
  const launchActionLabel = launch.resolved.requiresSlotLogin
    ? "Launch Codex Login"
    : "Launch Codex";
  const tokenWindows = createTokenWindowCollection(generatedAtMs);
  for (const account of accounts) {
    for (const preset of TOKEN_WINDOW_PRESETS) {
      mergeTokenWindowSnapshot(
        tokenWindows[preset.key],
        account.usage?.tokenWindows?.[preset.key],
      );
    }
  }
  finalizeTokenWindowCollection(tokenWindows);
  const allTimeWindow = tokenWindows.all;
  ...
  const summary = {
    ...
    staleCount: 0,
    emptyCount: 0,
    localTotalTokens: allTimeWindow.localTotalTokens,
    tokenBreakdown: allTimeWindow.tokenBreakdown,
    tokenCost: allTimeWindow.tokenCost,
    tokenWindows,
...
async function collectUsageStats(homeDir, options = {}) {
  ...
  const tokenWindows = createTokenWindowCollection(nowMs);
  ...
  for (const file of sessionFiles) {
    ...
    const usageLog = await readTokenUsageLog(file);
    if (!usageLog) {
      continue;
    }

    const fileActivityMs = stat?.mtime ? stat.mtime.getTime() : null;
    const tokenActivityMs = latestTimestampMs([
      usageLog.lastEventTimestampMs,
      fileActivityMs,
    ]);
    const tokenActivityAt =
      typeof tokenActivityMs === "number"
        ? new Date(tokenActivityMs).toISOString()
        : null;

    addUsageToTokenWindow(tokenWindows.all, usageLog.finalUsage, tokenActivityAt);
    for (const preset of TOKEN_WINDOW_PRESETS) {
      if (preset.key === "all") {
        continue;
      }
      const windowUsage = getWindowedTokenUsage(usageLog, preset.windowMs, nowMs, tokenActivityMs);
      if (windowUsage) {
        addUsageToTokenWindow(tokenWindows[preset.key], windowUsage, tokenActivityAt);
      }
    }
  }
  ...
  return {
    ...
    tokenBreakdown: allTimeWindow.tokenBreakdown,
    tokenCost: allTimeWindow.tokenCost,
    tokenFileCount: allTimeWindow.tokenFileCount,
    tokenWindows,
    latestActivityAt,
  };
}
...
// OpenAI API pricing per 1M tokens (USD), sourced from developers.openai.com/api/docs/pricing.
// Default model is gpt-5.4 (the Codex CLI default).
const MODEL_PRICING = {
...
const DEFAULT_PRICING_MODEL = "gpt-5.4";

// Legacy alias — points to the default model pricing.
const TOKEN_PRICING = {
  input_per_m: MODEL_PRICING[DEFAULT_PRICING_MODEL].input,
  cached_input_per_m: MODEL_PRICING[DEFAULT_PRICING_MODEL].cached,
  output_per_m: MODEL_PRICING[DEFAULT_PRICING_MODEL].output,
};

function estimateTokenCost(breakdown, model) {
  ...
  const cachedCost = (breakdown.cached_input_tokens / 1_000_000) * pricing.cached;
  const outputCost = (breakdown.output_tokens / 1_000_000) * pricing.output;
  ...
  return {
    ...
    savedByCaching: round2(
      ((breakdown.cached_input_tokens / 1_000_000) * pricing.input) - cachedCost,
    ),
  };
}
...
function normalizeTokenUsage(usage) {
  const normalized = {
    total_tokens: Math.max(0, toFiniteNumber(usage?.total_tokens) || 0),
    input_tokens: Math.max(0, toFiniteNumber(usage?.input_tokens) || 0),
    output_tokens: Math.max(0, toFiniteNumber(usage?.output_tokens) || 0),
    cached_input_tokens: Math.max(0, toFiniteNumber(usage?.cached_input_tokens) || 0),
    reasoning_output_tokens: Math.max(0, toFiniteNumber(usage?.reasoning_output_tokens) || 0),
  };
...
function subtractTokenUsage(current, baseline) {
  const left = normalizeTokenUsage(current);
  const right = normalizeTokenUsage(baseline);
  return {
    total_tokens: Math.max(0, left.total_tokens - right.total_tokens),
...
function addTokenUsageToBreakdown(target, usage) {
  if (!target || !usage) {
    return;
  }
...
function createTokenWindowSnapshot(preset, nowMs) {
  return {
    key: preset.key,
    label: preset.label,
    startAt:
      typeof preset.windowMs === "number"
        ? new Date(nowMs - preset.windowMs).toISOString()
        : null,
    endAt: new Date(nowMs).toISOString(),
    localTotalTokens: 0,
    tokenBreakdown: createEmptyTokenBreakdown(),
    tokenCost: null,
    tokenFileCount: 0,
...
function createTokenWindowCollection(nowMs) {
  return Object.fromEntries(
    TOKEN_WINDOW_PRESETS.map((preset) => [
      preset.key,
      createTokenWindowSnapshot(preset, nowMs),
    ]),
  );
}

function addUsageToTokenWindow(window, usage, activityAt) {
  if (!window || !usage) {
    return;
  }
  ...
  window.localTotalTokens += normalized.total_tokens;
  addTokenUsageToBreakdown(window.tokenBreakdown, normalized);
  window.tokenFileCount += 1;
  window.latestActivityAt = latestIso([window.latestActivityAt, activityAt]);
}

function mergeTokenWindowSnapshot(target, source) {
  if (!target || !source) {
    return;
  }
  target.localTotalTokens += source.localTotalTokens || 0;
  addTokenUsageToBreakdown(target.tokenBreakdown, source.tokenBreakdown);
  target.tokenFileCount += source.tokenFileCount || 0;
  target.latestActivityAt = latestIso([target.latestActivityAt, source.latestActivityAt]);
}

function finalizeTokenWindowCollection(tokenWindows) {
  if (!tokenWindows || typeof tokenWindows !== "object") {
    return tokenWindows;
  }
  for (const window of Object.values(tokenWindows)) {
    window.tokenCost = estimateTokenCost(window.tokenBreakdown);
  }
  return tokenWindows;
}
...
function extractTokenTimestampMs(row, eventPayload) {
  const candidates = [
    row?.timestamp,
    row?.created_at,
    row?.createdAt,
    ...
    eventPayload?.time,
    eventPayload?.ts,
    eventPayload?.info?.timestamp,
    eventPayload?.info?.created_at,
  ];
...
async function readTokenUsageLog(filePath) {
  ...
  try {
    for await (const line of rl) {
      ...
      if (usage && typeof usage.total_tokens === "number") {
        const normalized = normalizeTokenUsage(usage);
        const timestampMs = extractTokenTimestampMs(row, eventPayload);
  ...
  return {
    ...
    events: events
      .filter((event) => typeof event.timestampMs === "number")
      .sort((left, right) => left.timestampMs - right.timestampMs),
    hasCompleteTimestamps: events.length > 0 && !hasMissingTimestamps,
    lastEventTimestampMs: latestTimestampMs(
      events.map((event) => event.timestampMs).filter((value) => typeof value === "number"),
    ),
  };
}

function getWindowedTokenUsage(log, windowMs, nowMs, fallbackActivityMs) {
  if (!log?.finalUsage || typeof windowMs !== "number") {
    return null;
  }
  const startMs = nowMs - windowMs;
  if (log.hasCompleteTimestamps && Array.isArray(log.events) && log.events.length > 0) {
    let baseline = null;
    let hasWindowEvent = false;
    for (const event of log.events) {
      if (event.timestampMs < startMs) {
        baseline = event.usage;
        continue;
      }
      hasWindowEvent = true;
    }
    return hasWindowEvent
      ? subtractTokenUsage(log.finalUsage, baseline)
      : null;
  }
  return typeof fallbackActivityMs === "number" && fallbackActivityMs >= startMs
    ? log.finalUsage
    : null;
}

async function readFinalTokenUsage(filePath) {
  const usageLog = await readTokenUsageLog(filePath);
  return usageLog ? usageLog.finalUsage : null;
}
...
function toEpochMs(value) {
  ...
  if (value instanceof Date) {
    return Number.isNaN(value.valueOf()) ? null : value.getTime();
  }
  if (typeof value === "number" && Number.isFinite(value)) {
