...
function buildCodexLoginCommand(name, options = {}) {
  ...
  const cwd = options.cwd || getProjectHome();
  ...
  return {
    ...
    accounts: accounts.map((account) => ({
      ...
      commands: {
        ...
        launchLogin: buildCodexLoginCommand(account.slug, { projectHome }),
      },
    })),
    emptyState: {
...
async function collectUsageStats(homeDir, options = {}) {
  ...
  for (const file of sessionFiles) {
    ...
    const finalTokenCount = await readFinalTokenCount(file);
    if (typeof finalTokenCount === "number") {
      tokenFileCount += 1;
      localTotalTokens += finalTokenCount;
    }
  }
  ...
  return {
    ...
    localTotalTokens: deep ? localTotalTokens : null,
    tokenFileCount,
    latestActivityAt,
  };
}
...
async function readFinalTokenCount(filePath) {
  ...
  try {
    for await (const line of rl) {
      ...
      const direct = row?.info?.total_token_usage?.total_tokens;
      const eventPayload = row?.payload?.type === "token_count" ? row.payload : null;
      const eventTotal = eventPayload?.info?.total_token_usage?.total_tokens;
      const total = [eventTotal, direct].find((value) => typeof value === "number");
